...
import filter from "../../utils/provider/filterProvider";
...
function ListMail({
  ...
  const curFilter = localStorage.getItem("curFilter");
  if (curFilter) {
    filter(curFilter, Data);
  }
              ...
              <>
                {GridView == 0 || GridView == 1 ? (
                  ...
                  <ListBigCard
                    username={data && data?.from[0]?.address}
                    body={data.subject}
                    subject={data && data.sender && data?.sender[0]?.name}
                    time={istoDay(data)}
                    isstarred={CheckFlag(data, "flagged")}
                    setisAnyMailOpen={setisAnyMailOpen}
                    setopenedmail={setopenedmail}
