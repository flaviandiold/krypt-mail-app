...
function ShowMail({
    ...
    for (let index = 0; index < message?.length; index++) {
      if (message[index]?.envelope?.messageId === openedmail?.messageId) {
        ...
        if (
          ...
          console.log(user, userHome, pathContents);
          pathContents.mailStatus.unseen = pathContents.mailStatus.unseen - 1;
          WriteFile(path.join(userHome, "conf", "conf.txt"), pathContents);
          RetrieveMail();
        }
  ...
  async function RetrieveMail() {
    console.log("works on view mail");
